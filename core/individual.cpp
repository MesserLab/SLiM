...
Individual::Individual(const Individual &p_original) : subpopulation_(p_original.subpopulation_), index_(p_original.index_), age_(p_original.age_),
...
Individual::Individual(Subpopulation &p_subpopulation, slim_popsize_t p_individual_index) : subpopulation_(p_subpopulation), index_(p_individual_index), age_(0),
...
void Individual::GetGenomes(Genome **p_genome1, Genome **p_genome2) const
{
	...
	else
	{
		...
		// BCH 12 December 2017: changing to a raise here, since this should always be a fatal error
		EIDOS_TERMINATION << "ERROR (Individual::GetGenomes): (internal error) genome indices out of range." << EidosTerminate();
	}
...
EidosValue_SP Individual::GetProperty(EidosGlobalStringID p_property_id)
{
	...
	switch (p_property_id)
	{
		...
		{
			if (subpopulation_.population_.sim_.ModelType() == SLiMModelType::kModelTypeWF)

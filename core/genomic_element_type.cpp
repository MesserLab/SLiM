...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	{ 
		...
		// BCH 8/23/2026: In a change on 1/4/2026 I added filtering here to remove mutation types with a
		// proportion of 0.0.  In #663 it turned out that that broke backward compatibility in stdpopsim,
		// so I am backing that change out.  The motivation was to prevent mutation types with a fraction
		// of 0.0 from affecting optimization decisions; but that can be done by filtering them later on.
		mutation_types.emplace_back(mutation_type_ptr);
		mutation_fractions.emplace_back(proportion);
		
		// As per the comment above, this is one point where we filter "later"; but the muttype is added.
		if (proportion > 0.0)
		{
			...
			// see also Species::RecalculateOptimizationFlags() for the same logic
			if (mutation_type_ptr->all_neutral_DES_)
				species_.species_no_neutral_mutations_ = false;
			else	// !mutation_type_ptr->all_neutral_DES_
				species_.species_all_neutral_mutations_ = false;
		}
	}

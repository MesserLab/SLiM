...
void _RunSLiMSimTests(std::string temp_path)
{
	...
	SLiMAssertScriptRaise(gen1_setup + "1 { sim.addSubpop('p7', 10); sim.addSubpop(7, 10); stop(); }", 1, 245, "used already", __LINE__);
	...
	// Test sim EidosDictionaryUnretained functionality: - (+)getValue(string$ key) and - (void)setValue(string$ key, + value)
...
void _RunSubpopulationTests(void)
{
	...
	SLiMAssertScriptStop(gen1_setup_p1 + "1 { if (size(p1.genomesNonNull) == 20) stop(); }", __LINE__);
	...
	SLiMAssertScriptRaise(gen1_setup_p1 + "1 { p1.genomesNonNull = p1.genomesNonNull[0]; stop(); }", 1, 265, "read-only property", __LINE__);
	...
	SLiMAssertScriptStop(gen1_setup_sex_p1 + "1 { if (size(p1.genomesNonNull) == 15) stop(); }", __LINE__);
	...
	SLiMAssertScriptRaise(gen1_setup_sex_p1 + "1 { p1.genomesNonNull = p1.genomesNonNull[0]; stop(); }", 1, 285, "read-only property", __LINE__);
	...
	// Test Subpopulation EidosDictionaryUnretained functionality: - (+)getValue(string$ key) and - (void)setValue(string$ key, + value)
...
{	
	...
	SLiMAssertScriptStop(gen1_setup_p1 + "1 { i = p1.individuals; if (size(i.genomesNonNull) == 20) stop(); }", __LINE__);
	...
	SLiMAssertScriptRaise(gen1_setup_p1 + "1 { i = p1.individuals; i.genomesNonNull = i[0].genomes[0]; stop(); }", 1, 284, "read-only property", __LINE__);
	...
	SLiMAssertScriptStop(gen1_setup_sex_p1 + "1 { i = p1.individuals; if (size(i.genomesNonNull) == 15) stop(); }", __LINE__);
	...
	SLiMAssertScriptRaise(gen1_setup_sex_p1 + "1 { i = p1.individuals; i.genomesNonNull = i[0].genomes[0]; stop(); }", 1, 304, "read-only property", __LINE__);
	...
	// Test optional pedigree stuff; note that without preventIncidentalSelfing=T relatedness could be higher than 0.5
	...
	std::string gen1_setup_norel("initialize() { initializeSLiMOptions(keepPedigrees=F, preventIncidentalSelfing=T); initializeMutationRate(1e-7); initializeMutationType('m1', 0.5, 'f', 0.0); initializeGenomicElementType('g1', m1, 1.0); initializeGenomicElement(g1, 0, 99999); initializeRecombinationRate(1e-8); } 1 { sim.addSubpop('p1', 10); } ");
	std::string gen1_setup_rel("initialize() { initializeSLiMOptions(keepPedigrees=T, preventIncidentalSelfing=T); initializeMutationRate(1e-7); initializeMutationType('m1', 0.5, 'f', 0.0); initializeGenomicElementType('g1', m1, 1.0); initializeGenomicElement(g1, 0, 99999); initializeRecombinationRate(1e-8); } 1 { sim.addSubpop('p1', 10); } ");

		...
		if (nucleotide_based && (nuc_based.size() > 0))
		{
			...
			else
			{
				...
				for (const Polymorphism *polymorphism : nuc_based)
				{
					...
					slim_effect_t dominance = mut_trait_info[0].dominance_coeff_UNSAFE_;	// can be NAN
...
const std::vector<EidosMethodSignature_CSP> *Haplosome_Class::Methods(void) const
{
	...
	if (!methods)
	{
		...
		methods->emplace_back((EidosInstanceMethodSignature *)(new EidosInstanceMethodSignature(gStr_sumOfMutationsOfType, kEidosValueMaskFloat | kEidosValueMaskSingleton))->AddIntObject_S("mutType", gSLiM_MutationType_Class)->AddIntStringObject_OSN(gStr_trait, gSLiM_Trait_Class, gStaticEidosValueNULL)->MarkDeprecated());
...
#pragma unused (p_method_id, p_target, p_arguments, p_interpreter)
	...
	// check that the same haplosome is not included more than once as a target, which we don't allow
			...
			if (target_run)
			{
				for (int mut_index = value_index; mut_index < mutations_count; ++mut_index)
				{
					...
					if (target_run->enforce_stack_policy_for_addition(mutation_block, mut_to_add))
					{
						...
						// BCH 7/19/2026: If the mutation was not already segregating, then it needs to be added
						// to the registry.  This is now possible, since the user can retain lost mutations.
						// Note that kFixedAndSubstituted and kRemovedWithSubstitution were checked for above.
						if (mut_to_add->state_ == MutationState::kLostAndRemoved)
						{
							chromosome->MutationRegistryAdd(mut_to_add, /* p_autogenerated */ false);
							pop.species_.NoteMutationAdded(mut_to_add);
						}
						
						// Note that the nonneutral cache will already be invalidated (or will be new and invalid).
					}
				}
			}
		}
		...
		for (int mut_parameter_index = 0; mut_parameter_index < count_to_add; ++mut_parameter_index)
		{
			...
			if (position / mutrun_length == mutrun_index)
			{
				...
				if (p_method_id == gID_addNewDrawnMutation)
				{
					...
					// The constructor used above is normally used for autogenerated mutations, and calls NoteAutogeneratedMutation().
					// However, this mutation is not autogenerated, and -- the key point -- might belong to a mutation type that is
					// not in use by autogenerated mutations, and has thus not been taken into account in the optimization flags for
					// the simulation.  So we need to call NoteMutationAdded() to notify the simulation about this.
					species->NoteMutationAdded(new_mut);
				}
...
#pragma unused (p_method_id, p_target, p_arguments, p_interpreter)
	...
	else
	{
		...
		// BCH 7/19/2026: FIXME MULTICHROM: This logic could probably be cleaned up now, as per the comment above.
		if (create_substitutions)
			chromosome->SetMutationRegistryNeedsCheck();
		...
		if (recording_tree_sequence_mutations)
		{
			...
			// BCH 7/19/2026: Note that if a removed mutation is removed entirely, it will become "retained
			// by the tree sequence" in RemoveAllFixedMutations() when it is removed from the registry.  If
			// it has been substituted (distinguished by its state being marked kRemovedWithSubstitution above),
			// it will not become "retained by the tree sequence"; substitutions are still referenced by
			// derived states, but are represented in the mutation metadata table by the Substitution object,
			// not the Mutation object.
		}

...
void SLiMSim::ReadProvenanceTable(table_collection_t *p_tables, slim_generation_t *p_generation, SLiMModelType *p_model_type)
{
	...
	*p_remembered_genome_count = (size_t)rem_count_ll;
	...
	else if (file_version_02.is_string() && (file_version_02 == "0.2"))
	{
		...
		{
			EIDOS_TERMINATION << "ERROR (SLiMSim::ReadProvenanceTable): error reading provenance value (file_version 0.2); this file cannot be read." << EidosTerminate();
		}
	}
	else
	{
...
#pragma unused (p_function_name, p_arguments, p_argument_count, p_interpreter)
	...
	for (int mut_type_index = 0; mut_type_index < mut_type_id_count; ++mut_type_index)
	{
		...
		if (nucleotide_based_ && !mutation_type_ptr->nucleotide_based_)
			EIDOS_TERMINATION << "ERROR (SLiMSim::ExecuteContextFunction_initializeGenomicElementType): in a nucleotide-based model, initializeGenomicElementType() requires all mutation types for the genomic element type to be nucleotide-based.  Non-nucleotide-based mutation types may be used in a nucleotide-based model, but they cannot be autogenerated by SLiM, and therefore cannot be referenced by a genomic element type." << EidosTerminate();
...
#pragma unused (p_function_name, p_arguments, p_argument_count, p_interpreter)
	...
	if (nucleotide_based && !nucleotide_based_)
		EIDOS_TERMINATION << "ERROR (SLiMSim::ExecuteContextFunction_initializeMutationType): initializeMutationTypeNuc() may be only be called in a nucleotide-based model." << EidosTerminate();

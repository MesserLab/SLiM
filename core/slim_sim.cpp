...
void SLiMSim::WriteProvenanceTable(table_collection_t *p_tables)
{
	...
	sprintf(provenance_str, "{\"program\"=\"SLiM\", \"version\"=\"%s\", \"file_version\"=\"%s\", \"generation\"=%d}",
			SLIM_VERSION_STRING, SLIM_TREES_FILE_VERSION, Generation());
...
void SLiMSim::ReadProvenanceTable(table_collection_t *p_tables, slim_generation_t *p_generation)
{
...
slim_generation_t SLiMSim::_InstantiateSLiMObjectsFromTables(EidosInterpreter *p_interpreter)
{
	...
	ReadProvenanceTable(&tables, &provinence_gen);

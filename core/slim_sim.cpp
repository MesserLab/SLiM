...
#define SLIM_TREES_FILE_VERSION   "0.2"
...
void SLiMSim::WriteProvenanceTable(table_collection_t *p_tables)
{
	...
	j["schema"] = "tskit-provenance";
	j["schema_version"] = "0.1";
	
	j["software"] = "SLiM";		// note this key was named "program" in provenance version 0.1...
	j["program"] = "SLiM";		// ...so we still write it as "program" too, for backward compatibility
	j["version"] = SLIM_VERSION_STRING;
	j["file_version"] = SLIM_TREES_FILE_VERSION;	// 0.2 for this provenance format, 0.1 for the old format above
	j["model_type"] = (ModelType() == SLiMModelType::kModelTypeWF) ? "WF" : "nonWF";
	j["generation"] = Generation();
	j["remembered_node_count"] = (long)remembered_genomes_.size();
...
void SLiMSim::ReadProvenanceTable(table_collection_t *p_tables, slim_generation_t *p_generation, size_t *p_remembered_genome_count, SLiMModelType *p_model_type)
{
	...
	if (file_version_str != "0.1")
		EIDOS_TERMINATION << "ERROR (SLiMSim::ReadProvenanceTable): this .trees file was generated by an unrecognized version of SLiM; this file cannot be read." << EidosTerminate();
	...
	*p_remembered_genome_count = (size_t)rem_count_ll;
	...
	for (; slim_record_index >= 0; --slim_record_index)
	{
		...
		try {
			...
			// for an entry to be acceptable, it has to have a "program": "SLiM" entry ("software" is ignored here)
			if (j["program"] != "SLiM")
				continue;
			
			// we have a valid entry that parses as JSON and states that it is a SLiM entry; proceed
			break;
		} catch (...) {
			continue;
		}
	...
	std::string program_str, file_version_str, model_type_str;
	long long gen_ll, rem_count_ll;
	
	try {
		program_str = j["program"];
		file_version_str = j["file_version"];
	...
	{
		EIDOS_TERMINATION << "ERROR (SLiMSim::ReadProvenanceTable): missing or malformed provenance value; this file cannot be read." << EidosTerminate();
	}
	
	// these keys are unused in SLiM: model, parameters, schema, schema_version, seed, software, version
	// they are present for reproducibility of simulation and for interoperability; SLiM itself doesn't care about them at all
	
	if (program_str != "SLiM")
		EIDOS_TERMINATION << "ERROR (SLiMSim::ReadProvenanceTable): this .trees file was not generated with correct SLiM provenance information; this file cannot be read." << EidosTerminate();
	
	if ((file_version_str != "0.1") && (file_version_str != "0.2"))
		EIDOS_TERMINATION << "ERROR (SLiMSim::ReadProvenanceTable): this .trees file was generated by an unrecognized version of SLiM; this file cannot be read." << EidosTerminate();
...
{
	size_t individual_count = p_ts->individuals.num_records;
...
void SLiMSim::__TabulateMutationsFromTables(std::unordered_map<slim_mutationid_t, ts_mut_info> &p_mutMap)
{
...
void SLiMSim::__TallyMutationReferencesWithTreeSequence(std::unordered_map<slim_mutationid_t, ts_mut_info> &p_mutMap, std::unordered_map<node_id_t, Genome *> p_nodeToGenomeMap, tree_sequence_t *p_ts)
{
	...
	vg = (vargen_t *)malloc(sizeof(vargen_t));
	int ret = vargen_alloc(vg, p_ts, MSP_16_BIT_GENOTYPES);
...
slim_generation_t SLiMSim::_InstantiateSLiMObjectsFromTables(EidosInterpreter *p_interpreter)
{
	...
	
	{
		...
		__TabulateMutationsFromTables(mutInfoMap);

...
#define SLIM_TREES_FILE_VERSION   "0.1"
...
void SLiMSim::WriteProvenanceTable(table_collection_t *p_tables)
{
	...
	sprintf(provenance_str, "{\"program\"=\"SLiM\", \"version\"=\"%s\", \"file_version\"=\"%s\", \"generation\"=%d, \"remembered_node_count\"=%ld}",
			SLIM_VERSION_STRING, SLIM_TREES_FILE_VERSION, Generation(), (long)remembered_genomes_.size());
...
void SLiMSim::ReadProvenanceTable(table_collection_t *p_tables, slim_generation_t *p_generation, size_t *p_remembered_genome_count)
{
	...
	int conv = sscanf(last_record_str.c_str(), "{\"program\"=\"%100[^\"]\", \"version\"=\"%100[^\"]\", \"file_version\"=\"%100[^\"]\", \"generation\"=%100[0-9], \"remembered_node_count\"=%100[0-9]}%n", program, version, file_version, generation, rem_count, &end_pos);
	
	if ((conv != 5) || (end_pos != (int)last_record_len))
	...
	if (file_version_str != SLIM_TREES_FILE_VERSION)
		EIDOS_TERMINATION << "ERROR (SLiMSim::ReadProvenanceTable): this .trees file was generated by a different version of SLiM; this file cannot be read." << EidosTerminate();
...
void SLiMSim::__TabulateSubpopulationsFromTreeSequence(std::unordered_map<slim_objectid_t, ts_subpop_info> &p_subpopInfoMap, tree_sequence_t *p_ts)
{
...
slim_generation_t SLiMSim::_InstantiateSLiMObjectsFromTables(EidosInterpreter *p_interpreter)
{
	...
	ReadProvenanceTable(&tables, &provinence_gen, &remembered_genome_count);

...
Species::Species(Community &p_community, slim_objectid_t p_species_id) : self_symbol_(gID_sim, EidosValue_SP(new (gEidosValuePool->AllocateChunk()) EidosValue_Object_singleton(this, gSLiM_Species_Class))),
    x_experiments_enabled_(false), model_type_(p_community.model_type_), community_(p_community), population_(*this), name_(gStr_sim), species_id_(p_species_id)
{
#ifdef SLIMGUI
...
void Species::ReadTreeSequenceMetadata(tsk_table_collection_t *p_tables, slim_tick_t *p_tick, slim_tick_t *p_generation, SLiMModelType *p_model_type, int *p_file_version)
{
	...
	*p_remembered_genome_count = (size_t)rem_count_ll;
	...
	// Previous formats: everything is in provenance
		...
		if (num_rows <= 0)
			EIDOS_TERMINATION << "ERROR (Species::ReadTreeSequenceMetadata): no SLiM metadata; this file cannot be read." << EidosTerminate();
		...
		else if (file_version_02.is_string())
		{
			...
			// when reading binary files, version >= 0.5 should not hit this code path, but it's harmless to test for those versions below...
			...
			{
				EIDOS_TERMINATION << "ERROR (Species::ReadTreeSequenceMetadata): error reading provenance value (file_version " << file_version_02 << "); this file cannot be read." << EidosTerminate();
			}
		}
		else
		{

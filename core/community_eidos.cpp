...
const std::vector<EidosFunctionSignature_CSP> *Community::ZeroTickFunctionSignatures(void)
{
	...
	if (!sim_0_signatures_.size())
	{
		...
		sim_0_signatures_.emplace_back((EidosFunctionSignature *)(new EidosFunctionSignature(gStr_initializeTrait, nullptr, kEidosValueMaskObject | kEidosValueMaskSingleton, gSLiM_Trait_Class, "SLiM"))->AddString_S("name")->AddString_S("type")->AddFloat_ON("baselineOffset", gStaticEidosValueNULL)->AddFloat_OS("individualOffsetMean", gStaticEidosValue_Float0)->AddFloat_OS("individualOffsetSD", gStaticEidosValue_Float0)->AddLogical_OS("directFitnessEffect", gStaticEidosValue_LogicalF)->AddLogical_OS("substitutionAccumulation", gStaticEidosValue_LogicalT));
		...
		else
		{
			...
			if (block->species_spec_ && ((block->type_ == SLiMEidosBlockType::SLiMEidosFitnessEffectCallback) || (block->type_ == SLiMEidosBlockType::SLiMEidosMutationEffectCallback)))
			{
				if (block->species_spec_->InsideTraitOrFitnessCalculation())
					EIDOS_TERMINATION << "ERROR (Community::ExecuteMethod_deregisterScriptBlock): fitnessEffect() and mutationEffect() callback script blocks may not be deregistered within the context of a call to demandPhenotype(), demandPhenotypeForIndividuals(), recalculateFitness(), calculateFitness(), or calculatePhenotype()." << EidosTerminate();
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	{
		if (block->species_spec_->InsideTraitOrFitnessCalculation())
			EIDOS_TERMINATION << "ERROR (Community::ExecuteMethod_rescheduleScriptBlock): fitnessEffect() and mutationEffect() callback script blocks may not be rescheduled within the context of a call to demandPhenotype(), demandPhenotypeForIndividuals(), recalculateFitness(), calculateFitness(), or calculatePhenotype()." << EidosTerminate();
		if (block->species_spec_->Active() && ((cycle_stage_ == SLiMCycleStage::kWFStage6CalculateFitness) || (cycle_stage_ == SLiMCycleStage::kNonWFStage3CalculateFitness)))

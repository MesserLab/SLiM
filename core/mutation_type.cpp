...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	if (!all_neutral_DES_)
	{
		// if our DES is non-neutral and we're used in a GenomicElementType, that implies that the simulation is
		// now non-neutral (since we expect that a non-neutral mutation will now be generated by the GEType)
		...
		// if our DES is non-neutral, we set muttype_all_neutral_mutations_ to false to track that we are non-neutral
		// see also MutationType::MutationType() for the same logic at init time
		muttype_all_neutral_mutations_ = false;
	}

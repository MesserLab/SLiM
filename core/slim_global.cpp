...
{
	EIDOS_TERMINATION << "ERROR (SLiMRaiseGenerationRangeError): value " << long_value << " for a generation index or duration is out of range." << std::endl << eidos_terminate();
}

void SLiMRaisePositionRangeError(int64_t long_value)
{
	EIDOS_TERMINATION << "ERROR (SLiMRaisePositionRangeError): value " << long_value << " for a chromosome position or length is out of range." << std::endl << eidos_terminate();
}

void SLiMRaiseObjectidRangeError(int64_t long_value)
{
	EIDOS_TERMINATION << "ERROR (SLiMRaiseObjectidRangeError): value " << long_value << " for a SLiM object identifier value is out of range." << std::endl << eidos_terminate();
}

void SLiMRaisePopsizeRangeError(int64_t long_value)
{
	EIDOS_TERMINATION << "ERROR (SLiMRaisePopsizeRangeError): value " << long_value << " for a subpopulation size, individual index, or genome index is out of range." << std::endl << eidos_terminate();
}

void SLiMRaiseUsertagRangeError(int64_t long_value)
{
	EIDOS_TERMINATION << "ERROR (SLiMRaiseUsertagRangeError): value " << long_value << " for a user-supplied tag is out of range." << std::endl << eidos_terminate();
}

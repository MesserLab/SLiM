...
#pragma unused (p_function_name, p_arguments, p_interpreter)
	...
	if (SLiM_verbosity_level >= 1)
	{
		...
		else
		{
			...
			if (mutation_types.size() == 0)
				output_stream << ", integer(0)";
			else {
				...
				output_stream << ((mutation_types.size() > 1) ? ")" : "");
			}
			
			if (mutation_fractions.size() == 0)
				output_stream << ", integer(0)";
			else {
				bool first_mut_fraction = true;
				output_stream << ((mutation_fractions.size() > 1) ? ", c(" : ", ");
				for (double mut_fraction : mutation_fractions) {
					output_stream << (first_mut_fraction ? "" : ", ") << mut_fraction;
					first_mut_fraction = false;
				}
				output_stream << ((mutation_fractions.size() > 1) ? ")" : "");
			}
...
#pragma omp critical (GetProperty_periodicity_cache)
		...
		case gID__allocatedNonneutralCacheCount:
		{
			...
			// whether they are currently in use or in the freed pool; so this is a metric of past history too
		...
		case gID__inUseNonneutralMutationBufferSize:
		{
			...
			for (Chromosome *chromosome : chromosomes_)
			{
				...
				for (int context_index = 0; context_index < context_count; ++context_index)
				{
					...
					for (const MutationRun *mutrun : in_use_pool)
						if ((mutrun->nonneutral_cache_ != nullptr) && (mutrun->nonneutral_cache_->nonneutral_count_ > 0))
							allocated_size += mutrun->nonneutral_cache_->nonneutral_count_;
				}
			}
#endif
		...
		case gID__traitCalculationRegimeNameDIPLOID:
		case gID__traitCalculationRegimeNameHAPLOID:
		{
			TraitCalculationRegime trait_calculation_regime;
			
			if (p_property_id == gID__traitCalculationRegimeNameDIPLOID)
				trait_calculation_regime = current_trait_calculation_regime_DIPLOID_;
			else
				trait_calculation_regime = current_trait_calculation_regime_HAPLOID_;
			
			switch (trait_calculation_regime)
			{
				...
				case TraitCalculationRegime::kHaploidAllNonNeutralNoCallbacks:		return EidosValue_SP(new (gEidosValuePool->AllocateChunk()) EidosValue_String("kHaploidAllNonNeutralNoCallbacks"));
				case TraitCalculationRegime::kHaploidNoCallbacks:					return EidosValue_SP(new (gEidosValuePool->AllocateChunk()) EidosValue_String("kHaploidNoCallbacks"));
				case TraitCalculationRegime::kHaploidAllNonNeutralWithCallbacks:	return EidosValue_SP(new (gEidosValuePool->AllocateChunk()) EidosValue_String("kHaploidAllNonNeutralWithCallbacks"));
				case TraitCalculationRegime::kHaploidWithCallbacks:					return EidosValue_SP(new (gEidosValuePool->AllocateChunk()) EidosValue_String("kHaploidWithCallbacks"));
			}
		}
#endif
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	// TIMING RESTRICTION
	community_.EnforceTimingRestriction_EventBlockOnly("Species::ExecuteMethod_addSubpop", "addSubpop()", "");
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	// TIMING RESTRICTION
	community_.EnforceTimingRestriction_EventBlockOnly("Species::ExecuteMethod_addSubpopSplit", "addSubpopSplit()", "");
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	if (community_.executing_species_ == this)
		community_.EnforceTimingRestriction_EventBlockOnly("Subpopulation::ExecuteMethod_killIndividuals", "killIndividuals()", ", when called on the currently executing species");
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	// it can have the side effect of running mutationEffect() callbacks, and those cannot nest inside
	// the execution of a different species.
	community_.EnforceTimingRestriction_EventBlockOnly("Species::ExecuteMethod_readFromPopulationFile", "readFromPopulationFile()", "");
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	// it can have the side effect of running mutationEffect() callbacks, and those cannot nest inside
	// the execution of a different species.
	community_.EnforceTimingRestriction_EventBlockOnly("Species::ExecuteMethod_recalculateFitness", "recalculateFitness()", "");
	if (InsideTraitOrFitnessCalculation())
		EIDOS_TERMINATION << "ERROR (Species::ExecuteMethod_recalculateFitness): recalculateFitness() cannot be called when trait/fitness calculation is already underway." << EidosTerminate();
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	EidosValue *start_value = p_arguments[4].get();
	EidosValue *end_value = p_arguments[5].get();
	EidosValue *trait_value = p_arguments[6].get();
	
	slim_objectid_t script_id = -1;			// used if id_value is NULL, to indicate an anonymous block
	std::string script_string = source_value->StringAtIndex_NOCAST(0, nullptr);
	slim_objectid_t mut_type_id = -1;
	slim_objectid_t subpop_id = -1;			// used if subpop_value is NULL, to indicate applicability to all subpops
	slim_trait_index_t trait_index = -1;	// used if trait_value is NULL, to indicate applicability to all traits
	slim_tick_t start_tick = ((start_value->Type() != EidosValueType::kValueNULL) ? SLiMCastToTickTypeOrRaise(start_value->IntAtIndex_NOCAST(0, nullptr)) : 1);
	slim_tick_t end_tick = ((end_value->Type() != EidosValueType::kValueNULL) ? SLiMCastToTickTypeOrRaise(end_value->IntAtIndex_NOCAST(0, nullptr)) : SLIM_MAX_TICK + 1);
	...
	// note that unlike other uses of GetTraitIndexFromEidosValue(), we do not want to pass NULL in because it is
	// actually allowed; mutationEffect() callbacks can target a single trait or all traits (but not in between)
	if (trait_value->Type() != EidosValueType::kValueNULL)
		trait_index = GetTraitIndexFromEidosValue(trait_value, "registerMutationEffectCallback");
	...
	new_script_block->mutation_type_id_ = mut_type_id;
	new_script_block->subpopulation_id_ = subpop_id;
	new_script_block->trait_index_ = trait_index;
	if (trait_index != -1)
		new_script_block->trait_identifier_ = Traits()[trait_index]->Name();
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	// TIMING RESTRICTION
	community_.EnforceTimingRestriction_FirstEventStageOnly("Species::ExecuteMethod_skipTick", "skipTick()", "; skipping ticks should be arranged before any portion of the cycle has occurred");
...
#pragma unused (p_method_id, p_arguments, p_interpreter)
	...
	// TIMING RESTRICTION
	community_.EnforceTimingRestriction_EventBlockOnly("Species::ExecuteMethod_treeSeqSimplify", "treeSeqSimplify()", "");
...
#pragma unused (p_method_id, p_interpreter)
	...
	// with retained individuals that will keep them retained.
...
#pragma unused (p_method_id, p_interpreter)
	...
	// TIMING RESTRICTION
	community_.EnforceTimingRestriction_EventBlockOnly("Species::ExecuteMethod_treeSeqOutput", "treeSeqOutput()", "");
...
std::vector<EidosPropertySignature_CSP> *Species_Class::Properties_MUTABLE(void) const
{
	...
	if (!properties)
	{
		...
		properties->emplace_back((EidosPropertySignature *)(new EidosPropertySignature(gStr__traitCalculationRegimeNameDIPLOID,			true,	kEidosValueMaskString | kEidosValueMaskSingleton)));
		properties->emplace_back((EidosPropertySignature *)(new EidosPropertySignature(gStr__traitCalculationRegimeNameHAPLOID,			true,	kEidosValueMaskString | kEidosValueMaskSingleton)));
#endif
...
const std::vector<EidosMethodSignature_CSP> *Species_Class::Methods(void) const
{
	...
	if (!methods)
	{
		...
		methods->emplace_back((EidosInstanceMethodSignature *)(new EidosInstanceMethodSignature(gStr_demandPhenotype, kEidosValueMaskVOID))->AddIntObject_N("subpops", gSLiM_Subpopulation_Class)->AddIntStringObject_ON(gStr_trait, gSLiM_Trait_Class, gStaticEidosValueNULL)->AddLogical_OS("forceRecalc", gStaticEidosValue_LogicalF));

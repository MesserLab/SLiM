...
// This struct keeps information related to MutationType logging.  Having it in a struct compartmentalizes it.
typedef struct MutationTypeLoggingInfo {
	bool mutation_logging_on_ = false;
	bool log_autogeneratedOnly_;
	bool log_meanOnly_;
	...
	double running_id_, running_muttype_id_, running_chromosome_id_, running_position_, running_nucleotide_,
	running_origin_tick_, running_subpop_id_, running_tag_;
	
	slim_mutationid_t *logged_id_ = nullptr;
	slim_objectid_t *logged_muttype_id_ = nullptr;
	int64_t *logged_chromosome_id_ = nullptr;
	slim_position_t *logged_position_ = nullptr;
	int8_t *logged_nucleotide_ = nullptr;
	slim_tick_t *logged_origin_tick_ = nullptr;
	slim_objectid_t *logged_subpop_id_ = nullptr;
	slim_usertag_t *logged_tag_ = nullptr;
	
	std::vector<slim_trait_index_t> logged_trait_indices;
	std::vector<TraitEffectLog> logged_traits_;
} MutationTypeLoggingInfo;
...
typedef struct EffectSizeDistributionInfo {
...
private:
	...
	// Mutation data logging: see MutationType::ExecuteMethod_logMutationData()
	MutationTypeLoggingInfo log_info_;
	
	// This adds p_mut to the logs above, as selected by the user
	void _LogMutationInfo(Mutation *p_mut);
...
public:
	...
	void FreeLoggingInfo(void);
	inline __attribute__((always_inline)) bool LoggingOn(void) const { return log_info_.mutation_logging_on_; }
	inline __attribute__((always_inline)) void LogMutationInfo_AUTOGENERATED(Mutation *p_mut) { _LogMutationInfo(p_mut); }
	inline __attribute__((always_inline)) void LogMutationInfo_SCRIPTED(Mutation *p_mut) { if (!log_info_.log_autogeneratedOnly_)  _LogMutationInfo(p_mut); }

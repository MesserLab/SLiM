...
In SLiM 6.0, metadata about mutations is no longer kept in the tree sequence mutation table.  That was inconvenient because the same mutation can be recorded multiple times, in multiple derived states, due to stacking.  That led to duplicated metadata for the same mutation, recorded at different times and thus potentially different from recording to recording.  To remedy this, and make things generally more coherent and efficient, we now record mutation metadata in a binary chunk of top-level metadata.  Each mutation is recorded there only once.  The table is generated at save time, with the final metadata state for each mutation, so it is all up-to-date and correct.  So, the tskit mutation table contains information about tskit mutations, the derived state of which may contain more than one SLiM mutation; and information about SLiM mutations is stored in top-level metadata, under "SLiM_mutation_list".

At save time, we record information about all mutations (past and present) whose IDs are referenced in the "derived state" column of the mutation table (and only these mutations). To do this, we need to make sure to maintain any mutation objects that we might need, and then when writing out the tree sequence we simply loop through the derived state column, recording information for any referenced mutations.  (The actual implementation is a bit more complex, since some derived states refer to substitutions, some to segregating mutations, and some to mutations that have been lost, each of which gets handled differently.)

We make sure we have the correct set of mutation objects as follows: we will definitely use all segregating mutations, and all substitutions; these are present anyhow.  In addition, some mutations need to have their information written out even though they are not segregating/substituted.  These are referred to as "retained by the tree sequence".  The design is that mutations get "retained by the tree sequence" whenever they are removed or lost (but not substituted) from the SLiM simulation.  When the last copy of a mutation is removed or lost from the SLiM simulation, and the mutation is thus removed from SLiM's "mutation registry", that mutation is not freed; instead, it is transferred to being "retained by the tree sequence" so that the mutation is persisted even though it doesn't exist in any extant individuals, thus preserving its metadata for posterity.  You can search for the suffix `_by_treeseq_` to see all the code in the project related to this; it is a fairly small amount.  In particular, in `mutation.h` there is a very long comment on the `retained_by_treeseq_` ivar that discusses further implementation details that are beyond the scope of this document.  (For example, it is possible for mutations to be transferred back from being "retained by the tree sequence" to being in SLiM's mutation registry, and there is a distinction made internally between mutations that are being "tracked" by the tree sequence versus those that are actually retained by it; the internal details are fairly tricky.  See that long comment in the code for more.)

When a .trees file is read back in, mutations referenced by a derived state entry that do not get turned into segregating mutations or substitutions instead get instantiated as mutation objects that are "retained by the tree sequence", so that they continue to be persisted across multiple save/load cycles.  (Note, however, that only mutations referenced by a derived state entry will be instantiated; entries in the top-level SLiM mutation metadata table that are unreferenced are ignored by SLiM, and are not carried over.)

This mechanism retains a certain number of unneeded mutations, which are cleaned out periodically either after simplification, or at save time (even if simplification was not performed).  In Species::_MarkAndSweepTrackedMutations() the code does a pass through all derived states and releases mutations retained by the tree sequence if they are no longer referenced.

On disk, in a .trees files, the metadata column in the tree sequence mutation table is now (beginning in SLiM 6) used for just one thing: to store the _same_ information as is kept in the derived state column, but in binary, not ASCII.  Recall that for SLiM, the derived state column contains a list of the (stacked) mutations at a site, identified by their mutation IDs, in ASCII separated by commas.  For a mutation event that results in mutation IDs 6, 17, and 28 being stacked together at a site, the derived state entry for that mutation would be the ASCII string "6,17,28".  The derived state needs to be in ASCII because that is required by tskit.  In .trees files saved by SLiM 6, the same information is now stored in the mutation table metadata column as well, but in binary; so the mutation table's metadata for this example mutation would consist of three 64-bit ints: 6, 17, and 28.  This has no count or terminating NULL; instead, the number of mutation IDs stored is inferred by the length of the metadata entry.  This design is deliberately redundant, because pyslim's convert_alleles() function can convert the derived state column to using nucleotides instead.  That would lose the mutation ID information about the derived state, which is needed to look up the mutation metadata in the top-level mutation metadata table; but since the mutation IDs are _also_ in the mutation table's metadata column, the mutation ID information is preserved.  When reading a .trees file from disk, the binary mutation IDs in the metadata column are used to reconstruct the genetics of the tree sequence; the derived state column is ignored by SLiM on load, and is disposed of without checking that it matches the binary metadata.  This behavior could change in future, so it is probably best for these two redundant sources of information to be in exact agreement; but right now SLiM does not, in fact, check or care.

Note that in memory, while SLiM is running, the situation is a bit different; the metadata column of the mutation table is not used, and the derived state column contains the _binary_ mutation IDs for the derived states.  At save time, SLiM therefore shifts the binary derived state information over to the metadata column and generates the corresponding ASCII derived state strings; and at load time, the ASCII derived state strings are discarded and the binary derived state information is shifted back over into the derived state column.  This allows SLiM to handle the derived state information in binary while it is running (which is more efficient and convenient), while still providing tskit with the ASCII derived state information it requires.

...
#pragma unused (p_sound_name)
	...
	if (console_fd == -1)
	{
		...
		been_here = true;
		return "#WARNING (ExecuteFunctionCall): function beep() could not open /dev/tty0 or /dev/vc/0 for writing; output stream may contain control characters to produce beeps.";
	}

...
EidosValue_SP EidosInterpreter::Evaluate_Subset(const EidosASTNode *p_node)
{
	...
	else
	{
		...
		if (second_child_type == EidosValueType::kValueLogical)
		{
			...
			const std::vector<eidos_logical_t> &logical_index_vec = ((EidosValue_Logical *)second_child_value.get())->LogicalVector();
	...
	else	// (!simpleIntegerRange)
	{
		...
		if (range_count > 0)
		{
			...
			else if (!assigns_index && (range_count > 1))
			{
				...
				{
					const std::vector<eidos_logical_t> &range_vec = ((EidosValue_Logical *)range_value.get())->LogicalVector();

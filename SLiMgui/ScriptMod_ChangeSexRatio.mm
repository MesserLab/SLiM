...
@implementation ScriptMod_ChangeSexRatio

- (NSString *)sheetTitle
{
	return @"Change Sex Ratio";
}
...
- (void)configSheetLoaded
{
	...
	[sexRatioTextField setStringValue:@"0.5"];
...
- (NSString *)scriptLineWithExecute:(BOOL)executeNow
{
	...
	NSString *newRatio = [sexRatioTextField stringValue];
	...
	if (executeNow)
	{
		...
		else
		{
			...
			Event *new_event_ptr = new Event('X', event_parameters);
			
			controller->sim->events_.insert(std::pair<const int,Event*>(targetGeneration, new_event_ptr));
		}
	}
	
	return [NSString stringWithFormat:@"%d X p%d %@", targetGeneration, populationID, newRatio];
}

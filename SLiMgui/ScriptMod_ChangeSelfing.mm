...
- (IBAction)validateControls:(id)sender
{
	...
	[selfingRateTextField setBackgroundColor:(rateValid ? [NSColor whiteColor] : [ScriptMod validationErrorColor])];
...
- (NSString *)scriptLineWithExecute:(BOOL)executeNow
{
	...
	return [NSString stringWithFormat:@"%d S p%d %@", targetGeneration, populationID, newRate];
}

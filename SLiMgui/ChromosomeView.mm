...
- (void)dealloc
{
	...
	[_proxyMetalView setChromosomeView:nil];
	[self setProxyMetalView:nil];
...
- (void)setNeedsDisplayAll
{
	...
	if (_proxyMetalView)
		[_proxyMetalView setNeedsDisplay:YES];
}
...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	if (ready)
	{
		...
		if (!_proxyMetalView)
		{
			// If we have a proxy MTKView set up, then it does all of our interior drawing.
			// This is the old Cocoa drawing code, as a fallback and a historical record.
